...
of_error Hello::unpack(uint8_t* buffer) {
    ...
    uint32_t len = this->length_ - sizeof(struct ofp_fluid_header);
    uint8_t *p = buffer + sizeof(struct ofp_fluid_header);
    while (len) {
...
of_error FeaturesRequest::unpack(uint8_t *buffer) {
    OFMsg::unpack(buffer);
    if (this->length_ < sizeof(ofp_fluid_header)) {
        return openflow_error(of13::OFPET_BAD_REQUEST, of13::OFPBRC_BAD_LEN);
    }
    return 0;
}
...
of_error GetConfigRequest::unpack(uint8_t *buffer) {
    OFMsg::unpack(buffer);
    if (this->length_ < sizeof(struct ofp_fluid_header)) {
        return openflow_error(of13::OFPET_BAD_REQUEST, of13::OFPBRC_BAD_LEN);
    }
    return 0;
}
...
uint8_t* PortStatus::pack() {
    ...
    memset(ps->pad, 0x0, 7);
    this->desc_.pack(buffer + (sizeof(struct ofp_fluid_header) + 8));
    return buffer;
}
...
of_error PortStatus::unpack(uint8_t *buffer) {
    ...
    this->desc_.unpack(buffer + (sizeof(struct ofp_fluid_header) + 8));
    return 0;
}
