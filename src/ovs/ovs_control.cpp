...
// Copyright 2020 Futurewei Cloud - file modified.
//
    ...
    * -1 to let ovs-ofctl choose the default. */
    ...
    while (iss >> option) {
        if (option.compare("invalid_ttl") == 0) {
            uint32_t usable_versions = ((1u << OFP10_VERSION) | (1u << OFP11_VERSION) |
                                        (1u << OFP12_VERSION));
    ...
    while (iss >> option) {
        if (isdigit(option[0])) {
            ...
            /* inactive due to switching ovs dependency
    ...
    if (preferred_packet_in_format >= 0) {
        ...
        set_packet_in_format(
                vconn, static_cast<nx_packet_in_format>(preferred_packet_in_format), true);
    } else {
...
void OVS_Control::packet_out(const char *bridge, const char *options)
{
    ...
    error = parse_ofp_packet_out_str(&po, options, ports_to_accept(bridge), &usable_protocols);
    if (error) {
        //ovs_fatal(0, "%s", error);
                                       ...
                                       ofputil_protocol *protocolp)
{
    ...
    error = parse_ofp_flow_stats_request_str(fsr, aggregate, match, port_map, &usable_protocols);
    if (error) {
        //ovs_fatal(0, "%s", error);
        ACA_LOG_ERROR("%s", error);
    }
...
int OVS_Control::flow_mod(const char *bridge, const char *flow, unsigned short int command)
{
    ...
    error = parse_ofp_flow_mod_str(&fm, flow, ports_to_accept(bridge), command, &usable_protocols);
    if (error) {
        // ovs_fatal(0, "%s", error);
        ACA_LOG_ERROR("%s", error);
        rc = EXIT_FAILURE;
                                    ...
                                    size_t n_fms, enum ofputil_protocol usable_protocols)
{
    ...
    protocol = open_vconn_for_flow_mod(remote, &vconn, usable_protocols);
    usable_s = ofputil_protocols_to_string(protocol);
    ACA_LOG_INFO("vconn uses ofp protocol (%s)\n",
                  usable_s);
...
enum ofputil_protocol OVS_Control::open_vconn(const char *name, vconn **vconnp)
{
    ...
    // ovs_rundir() returns "/usr/local/var/run/openvswitch/", on some machine's ovs version it is not applicable
    ...
    sprintf(socket_name, "%s/%s.%s", "/var/run/openvswitch/", datapath_name, suffix);
    ACA_LOG_INFO("socket name is %s\n", socket_name);
    free(datapath_name);
    free(datapath_type);
    ...
    ACA_LOG_INFO("connecting to %s\n", vconn_get_name(*vconnp));
    ...
    struct X {
        ...
        static void ofctl_send(unixctl_conn *conn, int argc, const char *argv[], void *vconn_)
        {
            ...
            for (i = 1; i < argc; i++) {
                ...
                ofp_print(stderr, msg->data, msg->size,
                          ...
                          OVS_Control().ports_to_show(bridge), verbosity);
                                       ...
                                       const char *argv[], void *vconn_)
        {
            ...
            error_msg = parse_ofp_packet_out_str(
                    ...
                    OVS_Control().ports_to_accept(bridge), &usable_protocols);
            if (error_msg) {
                ds_put_format(&reply, "%s\n", error_msg);
                free(error_msg);
                ok = false;
    ...
    for (;;) {
        ...
        while (!blocked) {
            ...
            ofp_print(stderr, b->data, b->size,
                      ...
                      ports_to_show(bridge), verbosity + 2);
            fflush(stderr);
            ...
            case OFPTYPE_PACKET_IN:
                if (resume_continuations) {
                    ...
                    error = ofputil_decode_packet_in((ofp_header *)b->data, true,
                                                     NULL, NULL, &pin, &total_lenp,
...
bool OVS_Control::set_packet_in_format(vconn *vconn, enum nx_packet_in_format packet_in_format,
                                       bool must_succeed)
{
    struct ofpbuf *spif;

    //spif = ofputil_encode_set_packet_in_format(
    spif = ofputil_make_set_packet_in_format(
    ...
    if (must_succeed) {
        ...
        run(vconn_transact_noreply(vconn, spif, &reply), "talking to %s",
            vconn_get_name(vconn));
        if (reply) {
            char *s = ofp_to_string(reply->data, reply->size, NULL, 2);
                                      ...
                                      struct ovs_list *requests, const char *vconn_name)
{
    ...
    LIST_FOR_EACH_SAFE(error, next, list_node, errors)
    {
        ...
        if (!ofperr) {
            ...
            ofp_print(stderr, ofp_msg, msg_len, ports_to_show(vconn_name), verbosity + 1);
        }
...
void OVS_Control::port_iterator_fetch_features(port_iterator *pi)
{
    ...
    run(vconn_transact(pi->vconn, rq, &pi->reply), "talking to %s",
    ...
    for (;;) {
        if (ti->reply) {
            ...
            if (ti->variant == TI_STATS) {
                ...
                retval = ofputil_decode_table_features(ti->reply, &ti->features,
                                                       &ti->raw_properties);
            }
            if (!retval) {
                return &ti->features;
...
const ofputil_port_map *OVS_Control::get_port_map(const char *vconn_name)
{
    ...
    struct ofputil_port_map *map =
            (ofputil_port_map *)shash_find_data(&port_maps, vconn_name);
    if (!map) {
