...
void
OVS_Control::packet_out(const char *bridge, const char *options)
{
...
OVS_Control::set_protocol_for_flow_dump(vconn *vconn,
...
ofp_port_t
OVS_Control::str_to_port_no(const char *vconn_name, const char *port_name)
{
    ofp_port_t port_no;
...
bool
OVS_Control::try_set_protocol(struct vconn *vconn, enum ofputil_protocol want,
...
void
OVS_Control::fetch_switch_config(vconn *vconn, ofputil_switch_config *config)
{
...
void
OVS_Control::set_switch_config(vconn *vconn, const ofputil_switch_config *config)
{
...
int
OVS_Control::open_vconn_socket(const char *name, vconn **vconnp)
{
    char vconn_name[50];
    int error;
...
enum ofputil_protocol
OVS_Control::open_vconn(const char *name, vconn **vconnp)
{
...
int
OVS_Control::monitor_set_invalid_ttl_to_controller(vconn *vconn)
{
    struct ofputil_switch_config config;
...
const char *
OVS_Control::openflow_from_hex(const char *hex, ofpbuf **msgp)
{
...
void
OVS_Control::monitor_vconn(vconn *vconn, bool reply_to_echo_requests,
...
void
OVS_Control::run(int retval, const char *message, ...)
{
    if (retval) {
        va_list args;
...
bool
OVS_Control::set_packet_in_format(vconn *vconn,
 ...
 * Destroys 'request'. */
void
OVS_Control::transact_noreply(vconn *vconn, ofpbuf *request)
{
    struct ovs_list requests;
...
void
OVS_Control::transact_multiple_noreply(vconn *vconn, ovs_list *requests)
{
    struct ofpbuf *reply;
...
void
OVS_Control::send_openflow_buffer(vconn *vconn, ofpbuf *buffer)
{
    run(vconn_send_block(vconn, buffer), "failed to send packet to switch");
}

void
OVS_Control::dump_transaction(vconn *vconn, ofpbuf *request, const char *bridge)
{
...
bool
OVS_Control::str_to_ofp(const char *s, ofp_port_t *ofp_port)
{
    bool ret;
    uint32_t port_;
...
void
OVS_Control::port_iterator_fetch_port_desc(port_iterator *pi)
{
...
void
OVS_Control::port_iterator_fetch_features(port_iterator *pi)
{
...
void
OVS_Control::port_iterator_init(port_iterator *pi, vconn *vconn)
{
...
bool
OVS_Control::port_iterator_next(port_iterator *pi, ofputil_phy_port *pp)
{
    for (;;) {
        if (pi->reply) {
...
void
OVS_Control::port_iterator_destroy(port_iterator *pi)
{
    if (pi) {
 ...
 * '*pp'. */
void
OVS_Control::fetch_ofputil_phy_port(const char *vconn_name, const char *port_name,
                    ofputil_phy_port *pp)
{
    struct vconn *vconn;
    ofp_port_t port_no;
...
void
OVS_Control::table_iterator_init(table_iterator *ti, vconn *vconn)
{
...
OVS_Control::table_iterator_next(table_iterator *ti)
{
    for (;;) {
        if (ti->reply) {
            int retval;
...
void
OVS_Control::table_iterator_destroy(table_iterator *ti)
{
    if (ti) {
        while (ti->more) {
            /* Drain vconn's queue of any other replies for this request. */
...
OVS_Control::get_port_map(const char *vconn_name)
{
...
const ofputil_port_map *
OVS_Control::ports_to_accept(const char *vconn_name)
{
    return should_accept_names() ? get_port_map(vconn_name) : NULL;
}

const ofputil_port_map *
OVS_Control::ports_to_show(const char *vconn_name)
{
    return should_show_names() ? get_port_map(vconn_name) : NULL;
}

const ofputil_table_map *
OVS_Control::get_table_map(const char *vconn_name)
{
...
const ofputil_table_map *
OVS_Control::tables_to_accept(const char *vconn_name)
{
    return should_accept_names() ? get_table_map(vconn_name) : NULL;
}

const ofputil_table_map *
OVS_Control::tables_to_show(const char *vconn_name)
{
...
bool
OVS_Control::should_accept_names(void)
{
    return use_names != 0;
}
...
bool
OVS_Control::should_show_names(void)
{
