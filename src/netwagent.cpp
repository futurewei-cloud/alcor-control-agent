...
static void aca_cleanup()
{
	...
	if (dispatcher != NULL) //Currently is always NULL
	{
		...
		ACA_LOG_ERROR(
			"Unable to call delete, dispatcher pointer is null");
	}
	...
	if (async_grpc_server != NULL) {
		...
		ACA_LOG_ERROR(
			"Unable to call delete, async grpc server pointer is null.\n");
	}
	...
	if (async_grpc_server_thread != NULL) {
		if (async_grpc_server_thread->joinable()) {
			...
			ACA_LOG_ERROR(
				"Async grpc server thread is not joinable.\n");
		}
		...
		ACA_LOG_ERROR(
			"Unable to call delete, async grpc server thread pointer is null.\n");
	}
	ACA_LOG_CLOSE();
}
...
int main(int argc, char *argv[])
{
	...
	async_grpc_server_thread = new std::thread(
		std::bind(&Aca_Async_GRPC_Server::Run, async_grpc_server));

	MessageConsumer network_config_consumer(g_broker_list,
						g_kafka_group_id);
