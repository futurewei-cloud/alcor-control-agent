...
static void aca_convert_to_mac_array(const char *mac_string, u_char *mac)
{
	...
	if (sscanf(mac_string, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &mac[0],
	...
	for (int i = 0; i < parsed_struct.port_states_size(); i++) {
		...
		case aliothcontroller::OperationType::CREATE_UPDATE_SWITCH:
			...
			try {
				...
				my_ep_ip_address =
					current_PortConfiguration.fixed_ips(0).ip_address();
				// inet_pton returns 1 for success 0 for failure
				if (inet_pton(AF_INET, my_ep_ip_address.c_str(),
					      &(sa.sin_addr)) != 1) {
				...
				my_ep_mac_address =
					current_PortConfiguration.mac_address();
				...
				strncpy(veth_name, veth_name_string.c_str(),
					strlen(veth_name_string.c_str()) + 1);
					...
					if (parsed_struct.subnet_states(j).operation_type() ==
						...
						if (current_SubnetConfiguration.id() ==
							...
							if (parsed_struct
								...
								vpc_id = current_SubnetConfiguration
										 .vpc_id();

								my_gw_address =
									current_SubnetConfiguration
										.gateway_ip();

								my_cidr = current_SubnetConfiguration
										  .cidr();
				...
				ACA_LOG_DEBUG(
					"Endpoint Operation: %s: interface: %s, ep_ip: %s, mac: %s, hosted_interface: %s, veth_name:%s, tunid:%ld\n",
					aca_get_operation_name(
						parsed_struct.port_states(i).operation_type()),
					endpoint_in.interface, my_ep_ip_address.c_str(),
					current_PortConfiguration.mac_address().c_str(),
					endpoint_in.hosted_interface,
					endpoint_in.veth, endpoint_in.tunid);
		...
		case aliothcontroller::OperationType::FINALIZE:
			...
			try {
				...
				my_ep_host_ip_address =
					current_PortConfiguration.host_info().ip_address();
				// inet_pton returns 1 for success 0 for failure
				if (inet_pton(AF_INET, my_ep_host_ip_address.c_str(),
					      &(sa.sin_addr)) != 1) {
					throw std::invalid_argument("EP host ip address is not in the expect format");
				}
				...
				// the below will throw invalid_argument exceptions when it cannot convert the mac string
				aca_convert_to_mac_array(
					current_PortConfiguration.host_info()
						.mac_address()
						.c_str(),
					agent_md_in.ep.mac);
				...
				for (int j = 0;
					...
					if (parsed_struct.subnet_states(j).operation_type() ==
						...
						if (current_SubnetConfiguration.id() ==
							...
							agent_md_in.net.netip =
								sa.sin_addr.s_addr;

							my_prefixlen = my_cidr.substr(
								slash_pos + 1);
							// stoi throw invalid argument exception when it cannot covert
							agent_md_in.net.prefixlen =
								std::stoi(my_prefixlen);

							agent_md_in.net
								.switches_ips.switches_ips_len =
								current_SubnetConfiguration
									.transit_switches_size();
							uint32_t switches[RPC_TRN_MAX_NET_SWITCHES];
							agent_md_in.net
								.switches_ips.switches_ips_val =
								switches;

							for (int k = 0;
							     k < current_SubnetConfiguration
									 .transit_switches_size();
							     k++) {
								// inet_pton returns 1 for success 0 for failure
								if (inet_pton(AF_INET,
				...
				ACA_LOG_DEBUG(
					...
					aca_get_operation_name(
						parsed_struct.port_states(i).operation_type()),
					agent_md_in.ep.interface,
					my_ep_ip_address.c_str(),
					current_PortConfiguration.mac_address().c_str(),
					agent_md_in.ep.hosted_interface,
					agent_md_in.ep.veth, agent_md_in.ep.tunid);
			} catch (const std::invalid_argument &e) {
		...
		if ((rc == EXIT_SUCCESS) &&
			...
			rc = Aca_Net_Config::get_instance().create_veth_pair(
				temp_name_string, peer_name_string);
			if (rc == EXIT_SUCCESS) {
			...
			rc = Aca_Net_Config::get_instance().move_to_namespace(
				temp_name_string, namespace_name);
			if (rc == EXIT_SUCCESS) {
			...
			rc = Aca_Net_Config::get_instance().setup_veth_device(
				namespace_name, temp_name_string, my_ep_ip_address,
				my_prefixlen, my_ep_mac_address, my_gw_address);
			if (rc == EXIT_SUCCESS) {
					...
					if (parsed_struct.subnet_states(j).operation_type() ==
						...
						if (current_SubnetConfiguration.id() ==
							...
							for (int k = 0;
							     ...
							     k++) {
								ACA_LOG_DEBUG(
									"port operation: FINALIZE, update substrate, IP: %s, mac: %s\n",
									current_SubnetConfiguration
										.transit_switches(k)
										.ip_address()
										.c_str(),
								...
								rc = Aca_Net_Config::get_instance()
									     .rename_veth_device(
										     namespace_name, temp_name_string,
										     veth_name_string);
								if (rc == EXIT_SUCCESS) {
									ACA_LOG_INFO(
										"Successfully renamed in ns: %s, old_veth: %s, new_veth: %s\n",
										namespace_name
											.c_str(),
										temp_name_string
											.c_str(),
									...
									ACA_LOG_ERROR(
										"Unable to renamed in ns: %s, old_veth: %s, new_veth: %s\n",
										namespace_name
											.c_str(),
										temp_name_string
											.c_str(),
	...
	for (int i = 0; i < parsed_struct.subnet_states_size(); i++) {
		...
		case aliothcontroller::OperationType::CREATE_UPDATE_ROUTER:
			...
			try {
				...
				// inet_pton returns 1 for success 0 for failure
				if (inet_pton(AF_INET, my_ip_address.c_str(),
					      &(sa.sin_addr)) != 1) {
					throw std::invalid_argument("Ip address is not in the expect format");
				}
				...
				for (int j = 0;
				     j < current_SubnetConfiguration.transit_switches_size();
				     j++) {
					// inet_pton returns 1 for success 0 for failure
					if (inet_pton(AF_INET,
						      current_SubnetConfiguration
	...
	for (int i = 0; i < parsed_struct.vpc_states_size(); i++) {
		...
		case aliothcontroller::OperationType::CREATE_UPDATE_SWITCH:
			...
			try {
				...
				for (int j = 0;
				     j < current_VpcConfiguration.transit_routers_size();
				     j++) {
					// inet_pton returns 1 for success 0 for failure
					if (inet_pton(AF_INET,
...
int Aca_Comm_Manager::update_goal_state(const aliothcontroller::GoalState &parsed_struct)
{
	...
	rc = update_port_state(parsed_struct, transitd_command, transitd_input, exec_command_rc);
	if (rc == -EXIT_FAILURE) {
		ACA_LOG_ERROR("Failed to update port state. Failed with error code %d\n", rc);
	}
	rc = update_subnet_state(parsed_struct, transitd_command, transitd_input, exec_command_rc);
	if (rc == -EXIT_FAILURE) {
		ACA_LOG_ERROR("Failed to update subnet state. Failed with error code %d\n", rc);
	}
	rc = update_vpc_state(parsed_struct, transitd_command, transitd_input, exec_command_rc);
	if (rc == -EXIT_FAILURE) {
...
int Aca_Comm_Manager::execute_command(int command, void *input_struct)
{
	...
	ACA_LOG_INFO("Connecting to %s using %s protocol.\n",
		     g_rpc_server.c_str(), g_rpc_protocol.c_str());
	...
	if (g_debug_mode) {
		...
		case UPDATE_VPC: {
			...
			for (int i = 0;
			     i < rpc_trn_vpc_t_in->routers_ips.routers_ips_len; i++) {
		...
		case UPDATE_NET: {
			rpc_trn_network_t *rpc_trn_network_t_in =
				(rpc_trn_network_t *)input_struct;
			...
			for (int i = 0; i < rpc_trn_network_t_in->switches_ips.switches_ips_len;
			     i++) {
				ACA_LOG_DEBUG(
					"[execute_command] switches_ips_val[%d]: %u .\n",
					i, rpc_trn_network_t_in->switches_ips.switches_ips_val[i]);
			}
			break;
		}
		case UPDATE_EP: {
			rpc_trn_endpoint_t *rpc_trn_endpoint_t_in =
				(rpc_trn_endpoint_t *)input_struct;
			ACA_LOG_DEBUG(
				"[execute_command] Calling UPDATE EP with interface %s, ip %u, "
				"eptype %u, mac: %02x:%02x:%02x:%02x:%02x:%02x, hosted_interface: %s, "
				"veth: %s, tunid: %lu, remote_ips_len %u .\n",
				rpc_trn_endpoint_t_in->interface,
				rpc_trn_endpoint_t_in->ip, rpc_trn_endpoint_t_in->eptype,
			...
			for (int i = 0; i < rpc_trn_endpoint_t_in->remote_ips.remote_ips_len;
			     i++) {
				ACA_LOG_DEBUG(
					"[execute_command] remote_ips_val[%d]: %u .\n",
		...
		case UPDATE_AGENT_EP: {
			rpc_trn_endpoint_t *rpc_trn_endpoint_t_in =
				(rpc_trn_endpoint_t *)input_struct;
			ACA_LOG_DEBUG(
				"[execute_command] Calling UPDATE AGENT EP with interface %s, ip %u, "
				"eptype %u, mac: %02x:%02x:%02x:%02x:%02x:%02x, hosted_interface: %s, "
				"veth: %s, tunid: %lu, remote_ips_len %u .\n",
				rpc_trn_endpoint_t_in->interface,
				rpc_trn_endpoint_t_in->ip, rpc_trn_endpoint_t_in->eptype,
		...
		case UPDATE_AGENT_MD: {
			...
			rpc_trn_tun_intf_t *rpc_trn_tun_intf_t_in =
				&(rpc_trn_agent_metadata_t_in->eth);
			rpc_trn_endpoint_t *rpc_trn_endpoint_t_in =
				&(rpc_trn_agent_metadata_t_in->ep);
			rpc_trn_network_t *rpc_trn_network_t_in =
				&(rpc_trn_agent_metadata_t_in->net);
			...
			ACA_LOG_DEBUG(
				"[execute_command] Calling UPDATE AGENT MD with ep interface %s, ip %u, "
				"eptype %u, mac: %02x:%02x:%02x:%02x:%02x:%02x, hosted_interface: %s, "
				"veth: %s, tunid: %lu, remote_ips_len %u .\n",
				rpc_trn_endpoint_t_in->interface,
				rpc_trn_endpoint_t_in->ip, rpc_trn_endpoint_t_in->eptype,
	...
	if (client == NULL) {
		...
		case UPDATE_VPC:
			transitd_return =
				update_vpc_1((rpc_trn_vpc_t *)input_struct, client);
			break;
		case UPDATE_NET:
			transitd_return = update_net_1(
				(rpc_trn_network_t *)input_struct, client);
			break;
		case UPDATE_EP:
			transitd_return = update_ep_1(
				(rpc_trn_endpoint_t *)input_struct, client);
			break;
		case UPDATE_AGENT_EP:
			transitd_return = update_agent_ep_1(
				(rpc_trn_endpoint_t *)input_struct, client);
			break;
		case UPDATE_AGENT_MD:
			transitd_return = update_agent_md_1(
				(rpc_trn_agent_metadata_t *)input_struct, client);
			break;
		case DELETE_NET:
			// rc = DELETE_NET ...
			break;
		...
		case LOAD_TRANSIT_XDP:
			...
			break;
		case LOAD_TRANSIT_AGENT_XDP:
			transitd_return = load_transit_agent_xdp_1(
				(rpc_trn_xdp_intf_t *)input_struct, client);
			break;
	...
	ACA_LOG_INFO("Elapsed time for both RPC client create/destroy and transit "
		     ...
		     chrono::duration_cast<chrono::nanoseconds>(rpc_client_end - rpc_client_start)
			     .count(),
		     chrono::duration_cast<chrono::milliseconds>(rpc_client_end - rpc_client_start)
			     .count());
...
void Aca_Comm_Manager::print_goal_state(aliothcontroller::GoalState parsed_struct)
{
	...
	for (int i = 0; i < parsed_struct.port_states_size(); i++) {
		fprintf(stdout, "parsed_struct.port_states(%d).operation_type(): %d\n",
			i, parsed_struct.port_states(i).operation_type());
		...
		for (int j = 0; j < current_PortConfiguration.fixed_ips_size(); j++) {
			fprintf(stdout, "current_PortConfiguration.fixed_ips(%d): subnet_id %s, ip_address %s \n",
				j,
				current_PortConfiguration.fixed_ips(j)
					.subnet_id()
					.c_str(),
				current_PortConfiguration.fixed_ips(j)
					.ip_address()
		...
		for (int j = 0;
		     j < current_PortConfiguration.security_group_ids_size(); j++) {
			fprintf(stdout, "current_PortConfiguration.security_group_ids(%d): id %s \n",
				j,
				current_PortConfiguration.security_group_ids(j)
					.id()
					.c_str());
		}

		for (int j = 0;
		     j < current_PortConfiguration.allow_address_pairs_size(); j++) {
			fprintf(stdout, "current_PortConfiguration.allow_address_pairs(%d): ip_address %s, mac_address %s \n",
				j,
				current_PortConfiguration.allow_address_pairs(j)
					.ip_address()
					.c_str(),
				current_PortConfiguration.allow_address_pairs(j)
					.mac_address()
		...
		for (int j = 0;
		     j < current_PortConfiguration.extra_dhcp_options_size(); j++) {
			fprintf(stdout, "current_PortConfiguration.extra_dhcp_options(%d): name %s, value %s \n",
				j,
				current_PortConfiguration.extra_dhcp_options(j)
					.name()
					.c_str(),
				current_PortConfiguration.extra_dhcp_options(j)
					.value()
	...
	for (int i = 0; i < parsed_struct.subnet_states_size(); i++) {
		fprintf(stdout, "parsed_struct.subnet_states(%d).operation_type(): %d\n",
			i, parsed_struct.subnet_states(i).operation_type());
		...
		for (int j = 0;
		     j < current_SubnetConfiguration.transit_switches_size(); j++) {
	...
	for (int i = 0; i < parsed_struct.vpc_states_size(); i++) {
		fprintf(stdout, "parsed_struct.vpc_states(%d).operation_type(): %d\n",
			i, parsed_struct.vpc_states(i).operation_type());
		...
		for (int j = 0; j < current_VpcConfiguration.subnet_ids_size(); j++) {
			fprintf(stdout, "current_VpcConfiguration.subnet_ids(%d): %s \n",
				j, current_VpcConfiguration.subnet_ids(j).id().c_str());
		}
		...
		for (int l = 0; l < current_VpcConfiguration.transit_routers_size(); l++) {
			fprintf(stdout,
				...
				"%s \n",
				l,
				current_VpcConfiguration.transit_routers(l)
					.vpc_id()
					.c_str());
