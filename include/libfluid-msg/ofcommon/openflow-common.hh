...
OFP_ASSERT(sizeof(struct ofp_fluid_error_msg) == 12);

/* Switch configuration. */
struct ofp_fluid_switch_config {
    struct ofp_fluid_header header;
    uint16_t flags; /* OFPC_* flags. */
    uint16_t miss_send_len; /* Max bytes of new flow that datapath should
     send to the controller. */
};
OFP_ASSERT(sizeof(struct ofp_fluid_switch_config) == 12);
...
const uint16_t DESC_FLUID_STR_LEN = 256;
const uint8_t SERIAL_FLUID_NUM_LEN = 32;

/* Body of reply to OFPMP_DESC request. Each entry is a NULL-terminated
 * ASCII string. */
struct ofp_desc {
    char mfr_desc[DESC_FLUID_STR_LEN]; /* Manufacturer description. */
    char hw_desc[DESC_FLUID_STR_LEN]; /* Hardware description. */
    char sw_desc[DESC_FLUID_STR_LEN]; /* Software description. */
    char serial_num[SERIAL_FLUID_NUM_LEN]; /* Serial number. */
    char dp_desc[DESC_FLUID_STR_LEN]; /* Human readable description of datapath. */
};
OFP_ASSERT(sizeof(struct ofp_desc) == 1056);

/* Role request and reply message. */
struct ofp_role_request {
    struct ofp_fluid_header header; /* Type OFPT_ROLE_REQUEST/OFPT_ROLE_REPLY. */
...
struct ofp_async_config {
    struct ofp_fluid_header header; /* OFPT_GET_ASYNC_REPLY or OFPT_SET_ASYNC. */
...
const uint8_t OFP_FLUID_MAX_TABLE_NAME_LEN = 32;
...
const uint8_t OFP_FLUID_DEFAULT_MISS_SEND_LEN = 128;
/* Value used in "idle_timeout" and "hard_timeout" to indicate that the entry
 * is permanent. */
const uint8_t OFP_FLUID_FLOW_PERMANENT = 0;
/* By default, choose a priority in the middle. */
const uint16_t OFP_FLUID_DEFAULT_PRIORITY = 0x8000;
/* All ones is used to indicate all queues in a port (for stats retrieval). */
const uint32_t OFPQ_FLUID_ALL = 0xffffffff;
