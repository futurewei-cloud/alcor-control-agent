...
// copy from ovs source code "include/openvswitch/ofp-table.h"
struct ofputil_table_map {
    struct namemap map;
};

// copy from ovs source code "include/openvswitch/ofp-monitor.h"
char *parse_flow_monitor_request(struct ofputil_flow_monitor_request *,
                                 const char *, const struct ofputil_port_map *,
...
public:
    ...
    /* --unixctl-path: Path to use for unixctl server, for "monitor" and "snoop" commands. */
    char *unixctl_path;

    /*
     * structs and functions borrowed from ovs-ofctl.c
     */
    ...
    vconn *prepare_dump_flows(const char *bridge, const char *flow, bool aggregate,
                              ofputil_flow_stats_request *fsr, ofputil_protocol *protocolp);
    enum ofputil_protocol
    set_protocol_for_flow_dump(vconn *vconn, ofputil_protocol cur_protocol,
    ...
    bool set_packet_in_format(vconn *vconn, enum nx_packet_in_format packet_in_format,
                              bool must_succeed);
    ...
    void fetch_ofputil_phy_port(const char *vconn_name, const char *port_name,
                                ofputil_phy_port *pp);
