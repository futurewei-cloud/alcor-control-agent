...
. Gateway/Direct path. The path between the gateway node and the computing node is called the gateway path whereas VM to VM direct communication is called direct path. How to build gateway/direct path will be introduced later.
    ...
    .. The gateway node receieve the first packet and than send an OAM packet to the source host;
    .. The gateway node send the first packet (may be more than one due to delay) to the destination host;
    .. The source host receieve the OAM packet then build VM to VM direct path. After this, all packets go throuth the direct path;
...
. ACA receieves the message and extract information of ZGC;
...
This happens when a gateway node receieve the first packet from compute node, then send OAM packet to the computing node.
...
. The ovs on host receieves the OAM packet and pass it to ACA;
...
When ACA receieve OAM packet, it needs to parse it and decide what the action is.

. ACA receieves the OAM packet;
...
. br-int on Host receieves the packet. The source MAC of this packet is MAC_VM1, and the destination MAC is MAC_Gateway_Host1;

. br-int adds VLAN and sends the packet to ACA;

. ACA matches MAC_Gateway_Host1 and adds on-demand openflow rule to flow table, and sends the first packet back to br-int;

. br-int replaces the VLAN tag. Now the source MAC is MAC_Host1_DVR and the destination MAC is MAC_VM2;

. br_int sends the packet to br_tun;

. If there is no direct path, configure direct path vis gateway path. Otherwise, this packet will go through direct path;
...
. include/aca_zeta_state_handler.h
    .. Define the interface to receieve goal state message from Alcor server;
