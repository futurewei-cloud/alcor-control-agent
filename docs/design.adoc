...
. Customer creates new network(subnet)
	.. Allocate transit switch
		...
		... Go to s1, s2 and call “Update VPC” (VPC table)
...
==== Motivation
When making major changes to code, we want to version our components in such a way so that old clients have time to upgrade, and new clients can use the new features without issues.

==== Strategy
Our strategy is to have the two components explicitly state the API version in their messages.
Every message/call between the Controller and the Network Control Agent will have an API version tagged. +

Our components will support a range of different API versions by defining the max API version and min API version supported. +
We will increment each of these fields respectively as we upgrade +
also deprecate API versions.

For major version upgrades, we will

. Deploy changes to all Network Control Agents first
. Deploy changes to Network Controller once all Agents have been upgrades

==== Example
We want to support SR-IOV.
