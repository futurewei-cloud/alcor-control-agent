...
def talk_to_zeta(zgc_api_url, zeta_data):
    headers = {'Content-type': 'application/json'}
    # create ZGC
    ZGC_data = zeta_data["ZGC_data"]
    print(f'ZGC_data: \n{json.dumps(ZGC_data)}')

    # zgc_response = requests.post(zgc_api_url + "/zgcs", data=json.dumps(ZGC_data)).json()
    zgc_response = requests.put(zgc_api_url + "/zgcs/" + ZGC_data["zgc_id"], data=json.dumps(ZGC_data), headers=headers).json()
    print(f'zgc creation response: \n{zgc_response}')
    print(f'zgc_response type: \n{type(zgc_response)}')
    zgc_id = zgc_response['zgc_id']
    ...
    for node in zeta_data["NODE_data"]:
        ...
        print(f'node_data: \n{node_data}')
        node_response_data = requests.post(zgc_api_url + "/nodes", data=node_data, headers=headers)
    ...
    for tem in zeta_data["VPC_data"]:
        ...
        print(f'VPC_data: \n{VPC_data}')
        vpc_response_data = requests.post(zgc_api_url + "/vpcs", VPC_data, headers=headers).json()
    ...
    port_response_data = []
    for tem in PORT_data:
        port_data = json.dumps(tem)
        print(f'Port_data: \n{PORT_data}')
        port_response_data_tem = requests.post(zgc_api_url + "/ports", data=port_data, headers=headers).json()
        print(f'Response for adding port: {port_response_data_tem}')
        port_response_data.append(port_response_data_tem)
...
def run():
    # # Call zeta API to create ZGC,vpc etc.and generate the information ACA need, and save it in aca_data.json
    ...
    talk_to_zeta(zgc_api_url, zeta_data)
    ...
    # result1 = exec_sshCommand_aca(host=aca_nodes[0], user=aca_nodes_data['username'],
    #                               password=aca_nodes_data['password'], cmd=cmd_list1, timeout=60)
    ...
    # result2 = exec_sshCommand_aca(host=aca_nodes[1], user=aca_nodes_data['username'],
    #                               password=aca_nodes_data['password'], cmd=cmd_list2, timeout=60)
